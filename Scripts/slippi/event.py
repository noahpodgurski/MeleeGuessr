...
class Start(Base):
    ...
    @classmethod
    def find_subarray(cls, first_arr, second_arr):
        first_ptr = 0
        second_ptr = 0
    ...
    @classmethod
    def theSearch(cls, stream, streamTell, pattern, searchBuffer):
        stream.seek(max(0, streamTell-searchBuffer)) #go back BUFFER amt (50)
        buffer = []
        # print(pattern)
        for i in range(100):
            (byte,) = unpack('B', stream)
            buffer.append(byte)
            if cls.find_subarray(buffer, pattern):
                # print('FOUND IT')
    ...
    @classmethod
    def _parse(cls, stream, event_stream, bytes_read):
        slippi_ = cls.Slippi._parse(stream) #10
        ...
        stuff = stream.read(80)

        players = []
        for i in PORTS:
            
            (character, type, stocks, costume) = unpack('BBBB', stream) #4

            if (MAKE_COSTUMES_NEUTRAL and costume != 0):
                print(hex(character), hex(type), hex(stocks), hex(costume))
                target = 133
                OFFSET = 10+8+5+80+4-1


                # print(f"{OFFSET=}")
                # print(f"location: {bytes_read=}")
                # print(hex(stream.tell()))
                # print(f"diff from target is {stream.tell()-target}")
                # print(f"diff from manual calc is {stream.tell()-OFFSET}")

                #create pattern
                pattern = [x for x in stuff]
                pattern.append(character)
                pattern.append(type)
                pattern.append(stocks)
                pattern.append(costume)

                #search for pattern
                currLoc = event_stream.tell()
                # print(f"before: {currLoc}")

                # try with wider range first, less of a chance of a false positive
                costumePos = cls.theSearch(event_stream, stream.tell(), pattern, THE_SEARCH_BUFFER+80)
                if costumePos < 0:
                    #can't find wider range, do just the 4 bytes for char data
                    costumePos = cls.theSearch(event_stream, stream.tell(), (character, type, stocks, costume), THE_SEARCH_BUFFER)
                    if costumePos < 0:
                        raise Exception("Cannot parse costume from .slp file")
                # print(costumePos)

                #overwrite it (0)
                event_stream.seek(costumePos)
                event_stream.write(struct.pack('B', 0))
                event_stream.seek(currLoc)
                currLoc = event_stream.tell()
                # print(f"after: {currLoc}")
