...
def _parse_event(event_stream, payload_sizes, bytes_read):
    ...
    try:
        ...
        if event_type is EventType.GAME_START:
            event = Start._parse(stream, event_stream, bytes_read)
...
def _parse_events(stream, payload_sizes, total_size, handlers):
    ...
    while (total_size == 0 or bytes_read < total_size) and event != ParseEvent.END:
        ...
        (b, event) = _parse_event(stream, payload_sizes, bytes_read)
        bytes_read += b
        if isinstance(event, Start):
            ...
            if event.type is Frame.Event.Type.PRE or event.type is Frame.Event.Type.POST:
                ...
                current_frame.start = Frame.Start._parse(event.data, byte_read)
