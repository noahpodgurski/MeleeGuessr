...
class Config:
    def __init__(self):
        with open(THIS_CONFIG, 'r') as f:
            ...
            self.melee_iso_neut = os.path.expanduser(j['melee_iso_neut'])
            self.check_path(self.melee_iso_neut)
            self.gfx_config = os.path.expanduser(j['gfx_config'])
            self.check_path(self.gfx_config)
