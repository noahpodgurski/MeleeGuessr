...
import {
  ...
  parseGameStartEvent,
  firstVersion,
  handleFrameStartEvent,
  handleItemUpdateEvent,
  handlePostFrameUpdateEvent,
  handlePreFrameUpdateEvent,
  clearData,
  isInVersion,
} from "./parser/parser";
...
async function clipReplay(
  ...
  const length = readUint(rawData, 32, firstVersion, firstVersion, 0x0b);
  ...
  while (go && offset <= rawData.byteLength) {
    ...
    switch (command) {
      ...
      case 0x10:
        ...
        slicedData.set(
          new Uint8Array(rawData.buffer, offset, l + 1),
          slicedDataOffset
        );
      ...
      case 0x39: //game end
        ...
        slicedDataOffset += l + 0x01;
        console.log(
          `pushing GAME END at ${slicedDataOffset.toString(
            16
          )} (OG ${offset.toString(16)})`
        );
        break;
      case 0x3a: //frame start
        ...
        frameCount = handleFrameStartEvent(
          rawData,
          offset,
          replayVersion,
          frames
  ...
  console.log("================");
  console.log(
    `length data was set to ${length}, should be changed to ${slicedDataOffset}`
  );
  ...
  const fullData = Buffer.concat([
    ...
    new Uint8Array(
      rawData.buffer,
      offset,
      rawData.byteLength + rawData.byteOffset - offset
    ), //metadata
  ]);
...
export async function schliceSlp(
  inFile: fs.PathLike,
  outFile: fs.PathLike,
  endFrame: number
): Promise<number> {
...
function cutMetadataAndNames(rawData: DataView, offset: number): ArrayBuffer {
...
const readFileAndCutNamesAndMetadata = async (
  inFile: fs.PathLike,
  outFile: fs.PathLike
) => {
  const fd = await fs.promises.open(inFile, "r");
  ...
  const commandPayloadSizes = parseEventPayloadsEvent(
    rawData,
    data.raw.byteOffset
  );
